...
public class TileEntityEnergyHatch extends TileEntity
			implements IEnergySource,IEnergyConnection,IEnergyHandler,IChargeConductor{
	...
	public void updateEntity(){
		...
		if(Mods.Factorization.available)
			refreshCG();
		if(unit!=null && unit.equals(EnergyUnit.RF) && Mods.cofhApiEnergy.available)
			outputRF();
	}
	...
	public void displayInfoOnChat(EntityPlayer player){
		player.addChatComponentMessage(
				new ChatComponentTranslation("tile.CLGEnergyHatch.info", unit.unit));
	}
	...
	public void refreshCG(){
		...
		if(unit != null && unit==EnergyUnit.Charge){
			((Charge) this.charge).setValue((int) FZModule.getOutput(energyToOutput));
		}
